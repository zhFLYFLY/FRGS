...
class AirVLNSimulatorClientTool:
    ...
    def run_call(self, airsim_timeout: int = 60) -> None:
        ...
        def _run_command(index, socket_client: msgpackrpc.Client):
            ...
            result = socket_client.call('reopen_scenes', socket_client.address._host,
                                        self.machines_info[index]['open_scenes'])
            ...
            if scen_id in [1, 7]:
                ...
                while True:
                    try:
                        ...
                        if get_rgb:
                            ImageRequest.append(
                                airsim.ImageRequest(camera_id, airsim.ImageType.Scene, pixels_as_float=False,
                                                    compress=False)
                            )
                        if get_depth:
                            ImageRequest.append(
                                airsim.ImageRequest(camera_id, airsim.ImageType.DepthVis, pixels_as_float=False,
                                                    compress=True)
                            )
                ...
                while True:
                    try:
                        ...
                        if get_depth:
                            ImageRequest.append(
                                airsim.ImageRequest(camera_id, airsim.ImageType.DepthPerspective, pixels_as_float=True,
                                                    compress=False)
                            )
                        ...
                        if get_depth:
                            ...
                            depth_img_in_meters = airsim.list_to_2d_float_array(response_depth.image_data_float,
                                                                                response_depth.width,
                                                                                response_depth.height)
                            if depth_img_in_meters.min() < 1e4:
                                assert not (depth_img_in_meters.flatten()[
                                                0] == depth_img_in_meters).all(), 'DEPTH图片获取错误'

                            depth_img_in_meters = depth_img_in_meters.reshape(response_depth.height,
                                                                              response_depth.width, 1)
        ...
        def _getImages(airsim_client: airsim.VehicleClient, scen_id, get_rgb, get_depth, camera_id='front_0'):
            ...
            while True:
                try:
                    ...
                    if get_depth:
                        ImageRequest.append(
                            airsim.ImageRequest(camera_id, airsim.ImageType.DepthPlanar, pixels_as_float=True,
                                                compress=False)
                        )
                    ...
                    if get_depth:
                        assert response_depth.height == args.Image_Height_DEPTH and response_depth.width == args.Image_Width_DEPTH, 'DEPTH图片获取错误'
                        depth_img_in_meters = airsim.list_to_2d_float_array(response_depth.image_data_float,
                                                                            response_depth.width, response_depth.height)
                        if depth_img_in_meters.min() < 1e4:
                            assert not (depth_img_in_meters.flatten()[0] == depth_img_in_meters).all(), 'DEPTH图片获取错误'
                        depth_img_in_meters = depth_img_in_meters.reshape(response_depth.height, response_depth.width,
                                                                          1)
